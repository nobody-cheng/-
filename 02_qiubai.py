...
class QiubaiSpider:
    def __init__(self):
        ...
        self.headers= {"User-Agent":"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 Safari/537.36"}
    ...
    def get_content_list(self,html_str): #提取数据
        ...
        for div in div_list:
            ...
            item["hot_comment_author_name"] = item["hot_comment_author_name"][0].rsplit("：",1)[0] if len(item["hot_comment_author_name"])>0 else None
            #神评论内容
            item["hot_comment_content"] = div.xpath(".//div[@class='main-text']/text()")
            item["hot_comment_content"] = item["hot_comment_content"][0] if len(item["hot_comment_content"])>0 else None
            ...
            item["hot_comment_likenum"] = item["hot_comment_likenum"][0] if len(item["hot_comment_likenum"])>0 else None
