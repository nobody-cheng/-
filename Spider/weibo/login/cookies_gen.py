...
from weibo.header.headers import chiose_user_agent
...
user_agent = chiose_user_agent.get_ua()
...
def get_tid_and_c(post_url):
    ...
    try:
        ...
        raise ValueError('failed to gen cookies without login')
    else:
...
def get_cookies():
    ...
    try:
        ...
        if 'errline' in resp_str:
            raise ValueError('Invalid cookie without login')
