...
def handleNetwork(recvPack, ip):
    ...
    if recvPack[fqcore.cmd] == fqcore.sendfileack:
        # 创建线程,发送文件
        thread = threading.Thread(target=fqfile.sendFileThread, args=(recvPack[fqcore.filename], ip))
        thread.start()

    elif recvPack[fqcore.cmd] == fqcore.sendfile:
        sendfileMsg = {fqcore.cmd: fqcore.sendfileack, fqcore.filename: recvPack[fqcore.filename], fqcore.uuid: fqnet.uuid, fqcore.name: fqnet.name}
        fqnet.sendDict(sendfileMsg, ip)
...
def main():
    ...
    thread.start()
    # 启动文件接收线程
    thread = threading.Thread(target=fqfile.fileServerThread)
    thread.start()
    ...
    while True:
        ...
        if data.startswith('sendfile '):
            datas = data.split(' ', 2)
            if len(datas) < 3:
                print('  命令错误： sendfile {ip} {filename}')
                continue
            fqnet.sendfile(datas[1], datas[2])
